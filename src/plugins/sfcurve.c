...
/** sfcurve_compcb:
 * Compare two SfCurve objects.
 *
 * \param[in] o1  first SfCurve object
 * \param[in] o2  second SfCurve object
 *
 * \returns AY_TRUE if the curves are equal, AY_FALSE else
 */
int
sfcurve_compcb(ay_object *o1, ay_object *o2)
{
 sfcurve_object *s1, *s2;

  s1 = (sfcurve_object *)o1->refine;
  s2 = (sfcurve_object *)o2->refine;
...
Sfcurve_Init(Tcl_Interp *interp)
{
 ...
 int i, ops[3] = {AY_OPREVERT, AY_OPREFINE, AY_OPCOARSEN};
  ...
  ay_status += ay_comp_register(sfcurve_compcb, sfcurve_id);
