...
sdcurve_setpropcb(Tcl_Interp *interp, int argc, char *argv[], ay_object *o)
{
 ...
 Tcl_Obj *to = NULL;
 sdcurve_object *sdcurve = NULL;
 int update = AY_FALSE, new_closed, new_length = 0, new_level, new_type;
  ...
  if((to = Tcl_GetVar2Ex(interp, arr, "Level",
			 TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    {
...
/** sdcurve_compcb:
 * Compare two SDCurve objects.
 *
 * \param[in] o1  first SDCurve object
 * \param[in] o2  second SDCurve object
 *
 * \returns AY_TRUE if the curves are equal, AY_FALSE else
 */
int
sdcurve_compcb(ay_object *o1, ay_object *o2)
{
 sdcurve_object *s1, *s2;

  s1 = (sdcurve_object *)o1->refine;
  s2 = (sdcurve_object *)o2->refine;
  ...
  if(memcmp(s1->controlv, s2->controlv, 3*s1->length*sizeof(double)))
    return AY_FALSE;
...
Sdcurve_Init(Tcl_Interp *interp)
{
 ...
 int i, ops[5] = {AY_OPREVERT, AY_OPOPEN, AY_OPCLOSE,
  ...
  ay_status += ay_comp_register(sdcurve_compcb, sdcurve_id);
