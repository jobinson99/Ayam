...
objio_readface(char *str, int lastlinewasface)
{
 ...
 int texvlen = 0;
 ay_pomesh_object *po, *temppo;
 ay_tag *temptags;
 ay_object *m = NULL, *p = NULL;
 unsigned int nverts = 0, i;
 ...
 /*double angle, v1[3] = {0}, v2[3] = {0}, *v;*/
  ...
  if(lastlinewasface == -1)
      ...
      if(m)
	{
      ...
      /* clean up; make sure not to delete the first ay_object, as that
	 is currently linked to the scene... */
      tl = storedfaces->next;
      free(storedfaces);
      storedfaces = tl;
      while(storedfaces)
  ...
  if(!(p = calloc(1, sizeof(ay_object))))
    {ay_status = AY_EOMEM; goto cleanup;}
  ...
  po->npolys = 1;
  if(!(po->nloops = malloc(1 * sizeof(unsigned int))))
    {ay_status = AY_EOMEM; goto cleanup;}
  po->nloops[0] = 1;
  if(!(po->nverts = malloc(1 * sizeof(unsigned int))))
    {ay_status = AY_EOMEM; goto cleanup;}
  po->nverts[0] = 1;

  if(!(po->controlv = malloc(4*6*sizeof(double))))
    {ay_status = AY_EOMEM; goto cleanup;}
  ...
  while(*c != '\0')
      ...
      if(objio_checkdegen)
	{
      ...
      if((nverts + 1) * stride > 4*6)
	{
	  /* only get new memory, if initial size (which is sufficient
	     for a quad with normals) is exceeded */
	  newcontrolv = NULL;
  ...
  if(nverts >= 3 && !degen)
      ...
      if(nverts == 4)
	{
      ...
      */
      ...
      if(!(po->verts = malloc(nverts * sizeof(unsigned int))))
	{ ay_status = AY_EOMEM; goto cleanup; }
      ...
      for(i = 0; i < nverts; i++)
	{
      ...
      if(texv)
	{
	  ay_status = ay_pv_add(p, tcname, "varying", "g",
				texvlen, 2, texv);
	}
 ...
 Tcl_DString ds;
  ...
  switch(str[0])
	...
	ay_status = objio_readsurf(str);
      /* XXXX TODO else read smoothing group */
      break;
    case 't':
  ...
  if(str[0] == 'f')
    ...
    {
      if(str[0] == 'u' || (str[0] == 's' && str[1] == ' '))
	{
	  /* handle ignored usemtl/smoothing group statements */
	  if(lastlinewasface)
	    lastlinewasface = AY_TRUE;
	}
      else
