...
/** ay_tree_getclevel:
 * change the current level to the one given via a node specification,
 * properly maintaining \a ay_next; nothing changes if an error occurs
 *
 * \param[in] node  specification of a tree node, e.g. "root:1:2"
 *
...
ay_tree_getclevel(char *node)
{
 ...
 int i = 4, p, n;
 ay_object *o = ay_root;
 ay_list_object *oldlev = ay_currentlevel;
  ...
  /* set ay_next to end of new current level */
  if(o)
    {
      while(o->next)
	{
 ...
 *
 * \param[in] node  string in the form "root:1:2"
 *
 * \returns pointer to object or
 *          NULL if object does not exist or the string is wrongly encoded
 */
ay_object *
 ...
 *  create a tree structure in a Tcl string list
 *
 *  Implements the \a treeGetString scripting interface command.
 *  See also the corresponding section in the \ayd{sctreegetstring}.
 *
		    ...
		    int argc, char *argv[])
{
 ...
 Tcl_DString ds;
  ...
  if(argc != 3)
    {
      ay_error(AY_EARGS, argv[0], "varname node");
      return TCL_OK;
    }
...
/** ay_tree_isincurrentlevel:
 * Helper function that determines, whether a given object (\a o) is
 * in the current level.
 *
 * \param[in] o  object to check
 *
 * \returns AY_TRUE, if the object \a o is in the current level,
 *          AY_FALSE, else
 */
int
ay_tree_isincurrentlevel(ay_object *o)
{
 ay_object *l = ay_currentlevel->object;
...
/* ay_tree_selecttcmd:
 *  Tcl command to select objects given via tree node specification
 *  strings (e.g. "root:1:1").
 *
 *  Implements the \a treeSelect scripting interface command.
 *  See also the corresponding section in the \ayd{sctreeselect}.
 *
 * \returns TCL_OK in any case.
 */
int
		   ...
		   int argc, char *argv[])
{
 ...
 char vname[] = "ay(need_redraw)", yes[] = "1", no[] = "0";
  ...
  if(argc > 1)
	...
	{
	  ay_status = ay_tree_getclevel(argv[i]);
	  if(ay_status)
	    {
	      ay_error(AY_ERROR, argv[0], "Can not change current level.");
	      ay_selection = oldsel;
	      return TCL_OK;
	    }
	}
      ...
      while(i != argc)
	{
	  ...
	  if(o)
	    {
	      if(ay_tree_isincurrentlevel(o))
		{
		  ay_sel_add(o, /*set_selflag=*/AY_FALSE);
		}
	      else
		{
		  ay_error(AY_ERROR, argv[0],
			   "Can not select from different levels.");
		}
	    }
	  i++;
	}
  ...
  newsel = ay_selection;
  /* set selected flags to new selected objects */
  t = newsel;
  while(t)
    {
      t->object->selected = AY_TRUE;
      t = t->next;
    }
 ...
 int found = AY_FALSE;
  ...
  if(i == -1)
      ...
      else
	{
	  if(!p->parent)
	    {
	      ay_error(AY_ERROR, argv[0], "Drop target allows no children!");
	      Tcl_SetVar(interp, argv[3], "1", TCL_LEAVE_ERR_MSG);
	      return TCL_OK;
	    }
	}
