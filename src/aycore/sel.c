 ...
 long int argvi = 0;
  ...
  if(argc > 1)
      ...
      if(argv[1][0] == '-' && argv[1][1] == 'c')
	{
	 ...
	 (argv[1][0] == '-' && argv[1][1] == 'e'))
	{
	  ay_sel_hsltcmd(clientData, interp, 1, argv);
	  goto cleanup;
	}
      ...
      if(argv[1][0] == '-' && argv[1][1] == 'l' && argv[1][2] == 'b')
	{
	  if(argc < 3)
	    goto cleanup;
	  lbmode = AY_TRUE;
 ...
 *
 * This command is only available in the safe interpreter.
 *
 * Implements the \a sP scripting interface command.
 * See also the corresponding section in the \ayd{scsp}.
 *
 * \returns TCL_OK in any case.
 */
int
	       ...
	       int argc, char *argv[])
{
 ...
 ay_object *p;
  ...
  ay_sel_add(p, /*set_selflag=*/AY_TRUE);
