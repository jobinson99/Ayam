 ...
 *
 * \param[in] np  interpolation control surface, need not be compatible
 *                to \a st or \a en, will be sampled at \a w points in u
 *                and \a h points in v direction, the y coordinate
 *                of each sampled point will be used to control the
 *                interpolation of the corresponding points in
 *                \a st and \a en; the surface should therefore be defined
 *                in the XZ plane with Y between 0.0 and 1.0; however,
 *                these requirements are not enforced
 * \param[in] w  width of all 2D arrays
 * \param[in] h  height of all 2D arrays
 * \param[in] st  first 2D array to interpolate ([w*h*4], unchecked)
 * \param[in] en  second 2D array to interpolate ([w*h*4], unchecked)
 * \param[in,out] ta  target array, may be \a st, or \a en ([w*h*4], unchecked)
 *
 ...
 *
 * \param[in] nc  interpolation control curve, need not be compatible
 *                to \a st or \a en, will be sampled at \a len points,
 *                the y coordinate of each sampled point will be used
 *                to control the interpolation of the corresponding
 *                points in \a st and \a en;
 *                the curve should therefore be defined on the X axis
 *                with Y between 0.0 and 1.0;
 *                however, these requirements are not enforced
 * \param[in] len  length of all 1D arrays
 * \param[in] st  first 1D array to interpolate ([len*4], unchecked)
 * \param[in] en  second 1D array to interpolate ([len*4], unchecked)
 * \param[in,out] ta  target array, may be \a st, or \a en ([len*4], unchecked)
 *
 * \returns AY_OK on success, error code otherwise
 */
int
