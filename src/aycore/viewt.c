 ...
 *  setup the current OpenGL viewport and projection matrix to reflect
 *  the current "from" and "to" camera settings of a view;
 *  in addition, the position of the first light source is set
 *
 * \param[in] togl  Togl widget that represents the view to use
 *
 */
void
...
/* ay_viewt_zoomtoobj:
 * Togl callback to show all selected or all objects in a view window.
 *
 */
int
ay_viewt_zoomtoobj(struct Togl *togl, int argc, char *argv[])
{
 ...
 GLdouble mt[16] = {0};
  ...
  if((argc > 2) && !strcmp(argv[2], "-all"))
    {
      /* zoom to all requested... */
      if(view->type == AY_VTTRIM)
	{
	  /* zoom to all regardless of draw selection state in trim views */
	  zoom_to_all = AY_TRUE;
	}
      else
	{
	  /* only zoom to all if not drawing only selected objects
	     in non trim views */
	  if(!view->drawsel)
	    {
	      ay_selection = NULL;
  ...
  if(zoom_to_all && view->type == AY_VTTRIM)
    {
      i = ay_clevel_getparentbounds(bb);
      if(i)
	{
	  /* calc center of gravity (middle of bounding box) */
	  cog[0] = bb[0]+((fabs(bb[2]-bb[0]))/2.0);
	  cog[1] = bb[1]+((fabs(bb[3]-bb[1]))/2.0);
	  cog[2] = 0.0;

	  AY_V3SUB(dt, view->to, cog);

	  /* let the view point to COG */
  ...
  if(ay_selection)
      ...
      else
	{
	  ...
	  if(l < 1.0)
	    {
	      /* small object, back off a bit and use an adapted
		 zoom factor to avoid collisions with the frustum that
		 would make the object disappear */
	      AY_V3SCAL(dt, 2.0/lx);
	      ...
	      ly = ((l/2.0)/tan(view->zoom))/lx*2.1+1.0;
 ...
 char *tos[3] = {"cVToX", "cVToY", "cVToZ"};
  ...
  if(!(argc > 2))
      ...
      while(v)
	{
      ...
      if(v)
	{
	  printf("set createview to: %s\n", v->name);
	}
      */
      ay_prefs.createview = view;
    }
...
/* ay_viewt_setconftcb:
 *  Togl callback to configure/manipulate various aspects of a view
 */
int
	      ...
	      if(ay_selection || (argi == 3))
		{
		  switch(argi)
		      ...
		      if(ay_status)
			{
		      ...
		      break;
		    case 3:
		      ay_status = ay_viewt_markfromworld(togl, view->to);
		      if(ay_status)
			{
			  need_redraw = AY_FALSE;
			  ay_error(AY_ERROR, fname, NULL);
      ...
      else
	{
	  ...
	  if(view->type != AY_VTPERSP)
		...
		glVertex3i(-10000, 0, 10000);
	      ...
	      glDrawBuffer(GL_BACK);
	      glFlush();
	      ay_viewt_wintoworld(togl, winX1, winY1, &t[0], &t[1], &t[2]);
	      if(view->usegrid)
		{
		  t[0] = (ay_dround(t[0] / view->grid))*view->grid;
		  t[1] = (ay_dround(t[1] / view->grid))*view->grid;
		  t[2] = (ay_dround(t[2] / view->grid))*view->grid;
		}
	      memcpy(view->markworld, t, 3*sizeof(double));
	    }
	} /* if have picked point */
    } /* if 2 or 4 args */

  if(ay_prefs.normalizemark && ay_prefs.normalizedigits)
    {
      for(i = 0; i < 3; i++)
...
ay_viewt_markfromsel(struct Togl *togl)
{
 ...
 int i = 0, a = 0, numo = 0, numu;
  ...
  (void)ay_viewt_markfromworld(togl, cog);

 return AY_OK;
} /* ay_viewt_markfromsel */


/* ay_viewt_markfromworld:
 *  set mark from world coordinates
 */
int
ay_viewt_markfromworld(struct Togl *togl, double *world)
{
 ay_view_object *view = (ay_view_object *)Togl_GetClientData(togl);
 int i, height = Togl_Height(togl);
 GLint vp[4];
 GLdouble mm[16], mp[16], winx, winy, winz;
  ...
  if(GL_FALSE == gluProject(world[0], world[1], world[2], mm, mp, vp,
			    &winx, &winy, &winz))
    {
      return AY_ERROR;
    }
  ...
  AY_APTRAN3(view->markworld, world, mm);
