...
ay_shader_wrib(ay_shader *shader, int type, RtLightHandle *light_handle)
{
 ...
 int count = 0;
  ...
  if(shader->name)
	...
	case AY_STAREALIGHT:
	  *light_handle =
	    RiAreaLightSourceV(shader->name, count, tokens, values);
	  break;
	case AY_STINTERIOR:
		  ...
		  int argc, char *argv[])
{
  ...
  *shader = NULL;
  ...
  for(i = 0; i < sargnc; i++)
	  ...
	  for(j = 0; j < 3; j++)
	    {
	      ...
	      if((to = Tcl_GetVar2Ex(interp, arr, Tcl_DStringValue(&ds),
				     TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
		{
	...
	case AY_SASCALAR:
	  if((to = Tcl_GetVar2Ex(interp, arr, sargnv[i], TCL_LEAVE_ERR_MSG |
				 TCL_GLOBAL_ONLY)))
	    {
	      Tcl_GetDoubleFromObj(interp, to, &dtemp);
