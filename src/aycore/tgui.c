...
ay_tgui_update(Tcl_Interp *interp, int argc, char *argv[])
{
 ...
 ay_deletecb *cb = NULL;
 ay_voidfp *cbs = NULL;
 Tcl_WideInt numpolys = 0, numtris = 0, numquads = 0;
 unsigned int i;
 int use_tc = AY_FALSE, use_vc = AY_FALSE, use_vn = AY_FALSE;
 int smethod = 0, refine_trims = 0, primitives = 0;
 double sparamu = 0.0, sparamv = 0.0, quad_eps = 0.0;
 char infobuf[64+TCL_INTEGER_SPACE*3];
  ...
  while(o && oref)
      ...
      if(tmp)
	{
	  ...
	  po = (ay_pomesh_object*)(tmp->refine);
	  numpolys += po->npolys;
...
cleanup:
  ...
  i = snprintf(infobuf, sizeof(infobuf), "%Ld (tris: %Ld, quads: %Ld)",
	       numpolys, numtris, numquads);
  Tcl_SetVar2Ex(interp, arr, "NumPolys", Tcl_NewStringObj(infobuf,i),
...
/* ay_tgui_tcmd:
 * Tcl interface for tesselation GUI.
 * Implements the \a tguiCmd Tcl command.
 *
