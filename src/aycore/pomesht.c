		     ...
		     ay_pomesh_object *target)
{
 ...
 double *dtmp;
  ...
  /* fast forward m to the face to split off, also keep l for later use
 ...
 *
 * \param[in] po  polymesh object to generate the normals for
 ...
 *  The generated normal vectors will be normalized.
 *
 * \param[in,out] po  polymesh object to generate the normals for
 ...
 *
 * \param[in,out] po  polymesh object to remove the normals from
 *
 * \returns AY_OK on success, error code otherwise.
 */
int
 ...
 * Also implements the \a remsnPo scripting interface command.
 * Also implements the \a flipPo scripting interface command.
 * See also the corresponding section in the \ayd{scgenfnpo}.
 * See also the corresponding section in the \ayd{scgensnpo}.
 * See also the corresponding section in the \ayd{scremsnpo}.
 * See also the corresponding section in the \ayd{scflippo}.
 *
 * \returns TCL_OK in any case.
 */
int
 ...
 *
 * \param[in,out] po  polymesh object to process
 *
 * \returns AY_OK on success, error code otherwise.
 */
void
 ...
 *
 * \param[in] po  polymesh object to process
		   ...
		   ay_object **result)
{
 ...
 int stride = 3, isclosed1 = AY_TRUE, isclosed2 = AY_TRUE;
  ...
  ay_status = ay_pomesht_vertanglesums(pm1, &vas1);
...
ay_pomesht_selectbound(ay_pomesh_object *po, ay_point *selp)
{
 ...
 ay_point *cand = NULL, *scand = NULL, *p = NULL, *newp, **nextp;
  ...
  ay_status = ay_pomesht_vertanglesums(po, &vas);
  if(ay_status)
    return ay_status;
...
/** ay_pomesht_quadrangulate:
 * Refine a triangular polymesh, by inserting four new control points
 * for each original triangle and creating three quads for each original
 * triangle.
 *
 * \param[in] po  polymesh object (containing only triangles and no holes)
 *                to quadrangulate
 * \param[in,out] result  where to store the new polymesh object
 *
 * \returns AY_OK on success, error code otherwise.
 */
int
ay_pomesht_quadrangulate(ay_pomesh_object *po, ay_pomesh_object **result)
{
 int ay_status = AY_OK;
 /*char fname[] = "pomesht_quadrangulate";*/
 unsigned int i, j, a, b, stride = 3;
 unsigned int quads[] = {
   0, 1, 6, 5,
   1, 2, 3, 6,
   3, 4, 5, 6
 ...
 double p12[6], p23[6], p31[6], pbc[6];
  ...
  if(!ay_pomesht_hasonlyngons(po, 3))
    return AY_ERROR;
  ...
  if(!(npo->controlv = malloc(npo->npolys * 4 * stride * sizeof(double))))
    {
      ay_status = AY_EOMEM;
      goto cleanup;
    }
  npo->ncontrols = po->npolys * 7;
  npo->has_normals = po->has_normals;

  a = 0;
  b = 0;
  for(i = 0; i < po->npolys; i++)
    {
      p1 = &(po->controlv[po->verts[i*3]*stride]);
      p2 = &(po->controlv[po->verts[i*3+1]*stride]);
      p3 = &(po->controlv[po->verts[i*3+2]*stride]);

      memcpy(&(npo->controlv[a]), p1, stride*sizeof(double));
      a += stride;

      for(j = 0; j < stride; j++)
	p12[j] = p1[j]+(p2[j]-p1[j])*0.5;

      memcpy(&(npo->controlv[a]), p12, stride*sizeof(double));
      a += stride;

      memcpy(&(npo->controlv[a]), p2, stride*sizeof(double));
      a += stride;

      for(j = 0; j < stride; j++)
	p23[j] = p2[j]+(p3[j]-p2[j])*0.5;

      memcpy(&(npo->controlv[a]), p23, stride*sizeof(double));
      a += stride;

      memcpy(&(npo->controlv[a]), p3, stride*sizeof(double));
      a += stride;

      for(j = 0; j < stride; j++)
	p31[j] = p3[j]+(p1[j]-p3[j])*0.5;

      memcpy(&(npo->controlv[a]), p31, stride*sizeof(double));
      a += stride;

      for(j = 0; j < stride; j++)
	pbc[j] = p1[j]*1.0/3.0 + p2[j]*1.0/3.0 + p3[j]*1.0/3.0;

      memcpy(&(npo->controlv[a]), pbc, stride*sizeof(double));
      a += stride;
...
/** ay_pomesht_quadrangulatetcmd:
 *  Refine the selected triangular polymesh objects with a quadrangulation.
 *  Implements the \a quadPo scripting interface command.
 *  See also the corresponding section in the \ayd{scquadpo}.
 *
 *  \returns TCL_OK in any case.
 */
int
ay_pomesht_quadrangulatetcmd(ClientData clientData, Tcl_Interp *interp,
			     int argc, char *argv[])
{
 int ay_status = AY_OK;
 ay_list_object *sel = ay_selection;
 ...
 int notify_parent = AY_FALSE;
  ...
  while(sel)
      ...
      if(sel->object && sel->object->type == AY_IDPOMESH)
	{
	  ...
	  ay_status = ay_pomesht_quadrangulate(
				   (ay_pomesh_object *)o->refine, &np);

	  if(!ay_status && np)
	    {
	      ay_pomesht_destroy((ay_pomesh_object *)o->refine);
	      o->refine = np;
	    ...
	    {
	      if(np)
		ay_pomesht_destroy(np);
	      ay_error(AY_ERROR, argv[0], "Quadrangulation failed!");
	    }
	}
      else
	{
