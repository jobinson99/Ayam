...
/**ay_pv_add:
 *  add a new PV tag to object \a o
 *
 * \param[in,out] o  object to tadd the new PV tag to
 * \param[in] name  name of PV data (e.g. "myst")
 * \param[in] detail  detail string (e.g. "varying")
 * \param[in] type  data type string (e.g. "f", "g")
 * \param[in] datalen  number of data elements
 * \param[in] stride  distance between two data elements in \a data
 * \param[in] data  data array to read from (must be compatible to \a type)
 *
  ...
  if(buflen > comma4-comma1)
    {
	 ...
	 not run into this case for the next invocation) */
      memcpy(&(buf[buflen]), "   ", 3*sizeof(char));
      buflen += 3;
...
ay_pv_init(Tcl_Interp *interp)
{
  ...
  (void)ay_tags_register(ay_pv_tagname, &ay_pv_tagtype);
