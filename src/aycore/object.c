...
/** ay_object_placetcmd:
 *  Tcl command to place an object at designated point in space.
 *  Implements the \a placeOb scripting interface command.
 *  See also the corresponding section in the \ayd{scplaceob}.
 *
 *  \returns TCL_OK in any case.
 */
int
ay_object_placetcmd(ClientData clientData, Tcl_Interp *interp,
		    int argc, char *argv[])
{
/* int ay_status = AY_OK;*/
 int skip;
  ...
  while(sel)
    {
      ...
      while(tag)
	{
	  if(tag->type == ay_rp_tagtype)
	    {
	      if(!strcmp(tag->val, "RP"))
		{
		  skip = AY_TRUE;
		  break;
		}
 ...
 *  delete multiple objects that are connected via their ->next fields
 *
...
/** ay_object_place:
 *  move an object to the preferred initial place and orient it
 *
 * \param[in,out] o  object to process
 */
void
ay_object_place(ay_object *o)
{
 ay_list_object tsel = {0}, *osel;
 double tmp, yzlen, xzlen, dir[3], xrot, yrot;
 ...
 ay_view_object *view;
  ...
  if(ay_prefs.createat && ay_currentview)
    {
      if(ay_prefs.createat == 1 && ay_currentview->drawmark)
	{
      ...
      /*else fallback to viewpoint?*/

      if(ay_prefs.createat == 2)
	{
	  view = ay_prefs.createview;
	  if(!view)
	    view = ay_currentview;
	  if(!view)
	    return;
	  memcpy(pos, view->to, 3*sizeof(double));
	  if(ay_currentlevel->object != ay_root)
	    {
	      ay_trafo_identitymatrix(m);
	      ay_trafo_getparentinv(ay_currentlevel->next, m);
	      ay_trafo_apply3(pos, m);
	    }
	  if(view->grid != 0.0)
	    {
	      ay_pact_griddify(&(pos[0]), view->grid);
	      ay_pact_griddify(&(pos[1]), view->grid);
	      ay_pact_griddify(&(pos[2]), view->grid);
	    }

	  if(ay_prefs.normalizetrafos && ay_prefs.normalizedigits != 0)
	    {
	      pos[0] = ay_trafo_round(pos[0], ay_prefs.normalizedigits);
	      pos[1] = ay_trafo_round(pos[1], ay_prefs.normalizedigits);
	      pos[2] = ay_trafo_round(pos[2], ay_prefs.normalizedigits);
	    }
  ...
  if(ay_prefs.createin)
      ...
      while(v)
	{
      ...
      if(v)
	{
	  printf("using createview: %s\n", v->name);
	}
      */

      if(ay_prefs.createin == 1)
	{
	  /* align to view CS */
	  switch(view->type)
	    {
	    case AY_VTSIDE:
	      o->rotz = 90;
	      ay_quat_axistoquat(yaxis, 90*AY_PI/180.0, quat);
	      ay_quat_add(quat, o->quat, o->quat);
	      break;
	    case AY_VTTOP:
	      o->rotx = 90;
	      ay_quat_axistoquat(xaxis, 90*AY_PI/180.0, quat);
	      ay_quat_add(quat, o->quat, o->quat);
	      break;
	    case AY_VTFRONT:
	    case AY_VTTRIM:
      ...
      if(ay_prefs.createin == 2)
	{
	  /* align to camera trafo */
	  AY_V3SUB(dir, view->to, view->from);

	  if((fabs(dir[0]) < AY_EPSILON) &&
	     (fabs(dir[1]) < AY_EPSILON) &&
	     (fabs(dir[2]) < AY_EPSILON))
	    return;

	  xzlen = sqrt(dir[0]*dir[0] + dir[2]*dir[2]);
	  if(fabs(xzlen) < AY_EPSILON)
	    {
	      yrot = (dir[1] < 0.0) ? 180.0 : 0.0;
	    }
	  else
	    {
	      tmp = dir[2]/xzlen;
		...
		if(tmp >= 1.0)
		  ...
		  yrot = AY_R2D(acos(tmp));
	    }

	  yzlen = sqrt(dir[1]*dir[1] + xzlen*xzlen);
	  ...
	  if(fabs(xrot) > AY_EPSILON)
	    {
	      xrot = ay_trafo_round(xrot, ay_prefs.normalizedigits);
	      if(dir[1] < 0.0)
		{
		  xrot = -xrot;
		}
	      o->rotx += xrot;
	      ay_quat_axistoquat(xaxis, xrot*AY_PI/180.0, quat);
	      ay_quat_add(quat, o->quat, o->quat);
	    }

	  if(fabs(yrot) > AY_EPSILON)
	    {
	      yrot = ay_trafo_round(yrot, ay_prefs.normalizedigits);
	      if(dir[0] > 0.0)
		{
		  yrot = -yrot;
		}
	      o->roty += yrot;
	      ay_quat_axistoquat(yaxis, yrot*AY_PI/180.0, quat);
	      ay_quat_add(quat, o->quat, o->quat);
	    }
	} /* if */
    } /* if createin */
 ...
 *  Also implements the \a isHidden scripting interface command.
