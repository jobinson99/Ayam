...
##############################
# viewReset:
#  reset the camera (and roll and zoom) according to the current type
proc viewReset { w } {
 global ay
    ...
    if { $ay(cVType) != 3 } {
	$togl setconf -zoom 3.0 -roll 0.0 -cp 0.0 0.0
    } else {
	$togl setconf -zoom 0.25 -roll 0.0 -cp 0.0 0.0
    }
    $togl render
...
proc _getCmdFromPref { prefitem } {
 global ayprefs
    if { [subst \$$prefitem] != "" } {
	if { [_procExists [lindex [subst \$$prefitem] 0]] } {
	    return [subst \$$prefitem];
	} else {
	    ayError 1 "viewRender" "Configured cmd does not exist!"
	    set $prefitem ""
	}
    }
 return "";
...
proc viewRender { w type } {
	...
	set pre [_getCmdFromPref ayprefs(RenderPre)]
	set post [_getCmdFromPref ayprefs(RenderPost)]
    }
    if { $type == 1 } {
	set mode $ayprefs(QRenderMode)
	set pre [_getCmdFromPref ayprefs(QRenderPre)]
	set post [_getCmdFromPref ayprefs(QRenderPost)]
    }
    if { $type == 2 } {
	set pre [_getCmdFromPref ayprefs(FRenderPre)]
	set post [_getCmdFromPref ayprefs(FRenderPost)]
    }
...
proc viewOpen { width height {establish_bindings 1} {internal_view 0} } {
	...
	if { $lastid < 2 } {
	    ...
	    set w [frame .fv.fViews.f$name -takefocus 1 -highlightthickness $hl]
	} else {
...
proc viewClose { w } {
  global ay ayviewshortcuts
    ...
    set needus 0
    if { [string first "root:0" $ay(CurrentLevel)] == 0 } {
	goTop
	set ay(CurrentLevel) ""
	set needus 1
    }
...
proc viewSetMModeIcon { w mode } {
	...
	if { [lsearch [image names] $vimage] == -1 } {
	    image create photo $vimage
	}
	$vimage copy $image

	if { [lsearch [image names] overlayimg] == -1 } {
	    image create photo overlayimg -w 25 -h 25
	}
	overlayimg blank
	...
	if { $ay(cVPnts) } {
	    set col \#af0000
	    overlayimg put $col -to 20 1 24 5
	}
	...
	switch $type {
	    0 {
		# F
		overlayimg put $col -to 2 1 8 3
		overlayimg put $col -to 2 1 4 11
		overlayimg put $col -to 2 5 6 7
	    }
	    1 {
		# S
		overlayimg put $col -to 3 1 7 3
		overlayimg put $col -to 3 5 7 7
		overlayimg put $col -to 3 9 7 11
		overlayimg put $col -to 2 2 4 6
		overlayimg put $col -to 6 6 8 10
		overlayimg put $col -to 7 2 8 4
		overlayimg put $col -to 2 8 3 10
	    }
	    2 {
		# T
		overlayimg put $col -to 1 1 9 3
		overlayimg put $col -to 4 3 6 11
	    }
	    3 {
		# P
		overlayimg put $col -to 2 1 7 3
		overlayimg put $col -to 2 5 7 7
		overlayimg put $col -to 6 2 8 6
		overlayimg put $col -to 2 1 4 11
	    }
	}
	# switch
	...
	if { $ay(GuiScale) > 1.0 } {
	    ...
	    if { [lsearch [image names] overlayimgz] == -1 } {
		image create photo overlayimgz
	    }
	    overlayimgz copy overlayimg -zoom $rgs -compositingrule set
	    $vimage copy overlayimgz
	} else {
	    $vimage copy overlayimg
	}
...
proc viewSetPMode { w on } {
	...
	set s [expr round($ay(GuiScale))]
	$image put $col -to [expr 20*$s] [expr 1*$s] [expr 24*$s] [expr 5*$s]
    }
...
proc viewZoom { w d } {
 global ay ayprefs
    ...
    if { !$ayprefs(AutoFocus) && $ayprefs(FocusFollowsWheel) } {
	if { [focus] != $w } {
	    focus -force $w
	}
    }
...
proc viewMoveZ { w d } {
 global ay ayprefs
    ...
    set dx [expr $ay(cVFromX) - $ay(cVToX)]
    set dy [expr $ay(cVFromY) - $ay(cVToY)]
    set dz [expr $ay(cVFromZ) - $ay(cVToZ)]
    set dc [expr sqrt($dx*$dx + $dy*$dy + $dz*$dz) * $ayprefs(WheelMoveZ) * $d]

    set ntx [expr $ay(cVToX) + $dx * $dc]
    set nty [expr $ay(cVToY) + $dy * $dc]
    set ntz [expr $ay(cVToZ) + $dz * $dc]
    set nfx [expr $ay(cVFromX) + $dx * $dc]
    set nfy [expr $ay(cVFromY) + $dy * $dc]
    set nfz [expr $ay(cVFromZ) + $dz * $dc]

    $togl mc
    if { $ay(cVUndo) } {
	$togl setconf -undokb MoveZView -tox $ntx -toy $nty -toz $ntz\
	    -fromx $nfx -fromy $nfy -fromz $nfz
    } else {
	$togl setconf -tox $ntx -toy $nty -toz $ntz\
	    -fromx $nfx -fromy $nfy -fromz $nfz
    }
    update
    $togl reshape
...
##############################
# viewsZoomTo:
#  zoom all open views to selected or all objects
proc viewsZoomTo { {all ""} } {
 global ay
    if { $ay(views) == "" } {
	return;
    }
    undo save zoomAllViewsTo
    foreach view $ay(views) {
	$view.f3D.togl mc
	if { $all != "" } {
	    $view.f3D.togl zoomob -all
	} else {
	    $view.f3D.togl zoomob
	}
    }
    $ay(currentView) mc
 return;
