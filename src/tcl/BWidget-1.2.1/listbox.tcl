...
#  $Id: listbox.tcl,v 1.1 2001/10/10 16:15:31 randolf Exp $
# ------------------------------------------------------------------------------
#  Index of commands:
#     - ListBox::create
#     - ListBox::configure
...
proc ListBox::create { path args } {
    ...
    set data(hi) -1
    bind $path <Motion> "ListBox::_highlight $path %y"
    bind $path <Leave> "ListBox::_highlight $path -1"
...
proc ListBox::selection { path cmd args } {
    ...
    switch -- $cmd {
        ...
        toggle {
            foreach item $args {
		if { [lsearch $data(items) $item] != -1 } {
		    set idx [lsearch $data(selitems) $item]
		    if { $idx == -1 } {
			lappend data(selitems) $item
		    } else {
			set data(selitems) [lreplace $data(selitems) $idx $idx]
...
proc ListBox::_draw_item { path item x0 x1 y } {
    ...
    set txt [$path:cmd create text [expr {$x1+$indent}] $y \
    ...
    if { [set win [Widget::getoption $path.$item -window]] != "" } {
        ...
        set imgi [$path:cmd create image [expr {$x0+$indent}] $y \
		      -image $img -anchor w -tags "img i:$item"]
    }

    _createBGRect $path $item
}

proc ListBox::_getBGRectBB { path item } {
    set width [winfo width $path]
    set nbox [$path:cmd bbox "n:$item"]
    set y0 [lindex $nbox 1]
    set y1 [lindex $nbox 3]
    set ibox [$path:cmd bbox "i:$item"]
    if {[string compare "" $ibox]} {
	# image may exist and may be higher than text!
	set iy0 [lindex $ibox 1]
	set iy1 [lindex $ibox 3]
	set y0 [expr {$iy0<$y0?$iy0:$y0}]
	set y1 [expr {$iy1<$y1?$y1:$iy1}]
    }
    return [list 0 $y0 $width $y1];
}


proc ListBox::_createBGRect { path item } {
    set bb [_getBGRectBB $path $item]
    set bgrect [$path:cmd create rectangle $bb \
		    -outline [$path cget -background] \
		    -fill [$path cget -background] \
		    -tags "rect r:$item"]
    $path:cmd lower $bgrect n:$item
    $path:cmd bind $bgrect <ButtonPress-1> "ListBox::_toggle $path $item;"
}
...
proc ListBox::_redraw_items { path } {
    ...
    foreach item $data(upd,delete) {
        $path:cmd delete i:$item n:$item s:$item r:$item
    }
    foreach item $data(items) {
        $path:cmd delete r:$item
    }
    foreach item $data(items) {
        if { [info exists data(upd,create,$item)] } {
    ...
    foreach item $data(items) {
	if {[$path:cmd type r:$item] == "" } {
	    ListBox::_createBGRect $path $item
	}
    }
...
proc ListBox::_redraw_selection { path } {
	...
	set bb [_getBGRectBB $path $item]
	set bgr [eval $path:cmd create rectangle $bb \
		     -fill $selbg -outline $selbg -tags [list "sel s:$item"]]
	$path:cmd itemconfigure "n:$item" -fill $selfg
	$path:cmd lower $bgr
	$path:cmd bind $bgr <ButtonPress-1>\
	    "ListBox::selection $path toggle $item;"
	catch {$path:cmd lower "r:$item" $bgr}
    }
}
...
proc ListBox::_highlight { path y } {
    variable $path
    upvar 0  $path data
    set deltay [Widget::getoption $path -deltay]
    set hi [expr round(floor($y / ($deltay + 1)))]
    if { $data(hi) != $hi } {
	set item [lindex [$path items] $data(hi)]
	if { $item != "" } {
	    $path:cmd itemconfigure r:$item -outline ""
	}
	set item [lindex [$path items] $hi]
	if { $item != "" } {
	    $path:cmd itemconfigure r:$item -outline gray
	}
	set data(hi) $hi
    }
}

proc ListBox::_toggle { path item } {
    if { [$path selection get] == $item } {
	ListBox::selection $path clear
    } else {
	ListBox::selection $path clear
	ListBox::selection $path add $item
    }
}
