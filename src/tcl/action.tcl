...
proc actionEnd { n1 n2 op } {
	...
	notifyOb -mod
	catch {$ay(currentView) setconf -a 0}
    } else {
	catch {$ay(currentView) setconf -a 1}
    }
...
proc _setMarkBinding { w d } {
	if { [string first ".view" $w] == 0 } {
	    set w [winfo toplevel $w]
	}
	$w.f3D.togl mc
	$w.f3D.togl setconf -cmark $d
 return;
}
...
proc actionBindCenter { w { nextaction "" } } {
    ...
    bind $t $ayviewshortcuts(CenterO) "_setMarkBinding %W 0;"
    # set mark from selected points center of gravity
    bind $t $ayviewshortcuts(CenterPC) "_setMarkBinding %W 2;"
    # set mark from selected points bounding box center
    bind $t $ayviewshortcuts(CenterPB) "_setMarkBinding %W 1;"
    # set mark from view aim point (window center)
    bind $t $ayviewshortcuts(CenterV) "_setMarkBinding %W 3;"

    if { $nextaction != "" } {
	bind $t $ayviewshortcuts(CenterO) "+ $nextaction $w;"
	bind $t $ayviewshortcuts(CenterPC) "+ $nextaction $w;"
	bind $t $ayviewshortcuts(CenterPB) "+ $nextaction $w;"
	bind $t $ayviewshortcuts(CenterV) "+ $nextaction $w;"
    }
...
proc actionSetMark { w { nextaction "" } } {
    ...
    actionBindParamKbd $t ay(kbdmark) " Mark: " "setMark "
...
proc actionGetParamFromKbd { w var txt cmd key } {
	...
	bind $f.e <Key-Return> "$cmd \$$var;"
	bind $f.e <Shift-Key-Return> "$cmd \$$var;destroy $f;focus $w;"
    } else {
	bind $f.e <Key-Return> "$cmd \$$var;destroy $f;focus $w;"
	bind $f.e <Shift-Key-Return> "$cmd \$$var;"
    }
    bind $f.e <Key-KP_Enter> [bind $f.e <Key-Return>]
    bind $f.e <Shift-Key-KP_Enter> [bind $f.e <Shift-Key-Return>]
    foreach evt { <Key-Return> <Shift-Key-Return> <Key-KP_Enter>
	<Shift-Key-Return> } {
	bind $f.e $evt "+event generate $w.f3D.togl <ButtonRelease-1>;break;"
    }
...
proc actionClearKbd { w } {
    ...
    bind $w $ayviewshortcuts(CenterV) ""
    bind $w <Key-Return> ""

    shortcut_viewactions [winfo parent [winfo parent $w]]
