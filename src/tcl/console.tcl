...
#    set exp "\[^\\]\[ \t\n\r\[\{\"\$]"
	...
	default {
	    set ind [$w search -backwards -regexp "\[^\\\\\]\[\[ \t\n\r\{\"\]" insert-1c limit-1c]
	    if { $ind != "" } {
		set lstr [$w get $ind insert]
	    } else {
		set lstr ""
	    }
	    if { [string length $str] < 1 || [string first " " $lstr] == -1 } {
		set expandtypes { Procname Variable Pathname }
	    } else {
		set lastword \
	     [string range $lstr [string last " " $lstr] end]
		if { [string first "$" $lastword] == 1 } {
		    set expandtypes { Variable }
		} else {
		    if { [string first ":" $lastword] == 1 } {
			set expandtypes { Procname Variable }
		    } else {
			set expandtypes { Pathname Procname Variable }
		    }
		}
	    }
	    set res {}
	    foreach t $expandtypes {
