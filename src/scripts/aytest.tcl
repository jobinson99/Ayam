...
array set aytestprefs {
    ...
    RandomOrder 0
    AdvancedOptions 0
    TestProcs { testDefaultCallbacks testValidSolidVariations testValidNURBS testValidToolObjects testModellingTools testAllSolidVariations testCustomObjects testScriptObjects testInterrogatingObjects }
}
...
proc aytest_selectGUI { } {
    ...
    $lb insert end "Test 9 - Interrogating Objects"
...
# Gordon Variation #2 (triangular Gordon)
array set Gordon_2 {
    precmd {
	crtOb NCurve;
	crtOb NCurve; hSL; rotOb 0 0 -90;
	setPnt -world 3 0.5 1 0.2 1;
	crtOb NCurve; hSL; rotOb 0 0 -90; movOb 0.75 0 0;
	setPnt -world 3 0.5 1 0.2 1;
	hSL 3; cutOb;
	goDown -1;
...
array set ApproxNPU {
    types { NPatch }
    command { approxNP -m 0 -tp 2 }
}

array set ApproxNPV {
    types { NPatch }
    command { approxNP -m 1 -tp 2 }
}

array set ApproxNPUV {
    types { NPatch }
    command { approxNP -m 2 -tp 2 }
}

array set ApproxNPVU {
    types { NPatch }
    command { approxNP -m 3 -tp 2 }
}
...
#
# Test Interrogating Objects
#
proc testInterrogatingObjects { tools } {
set ::aytesttools $tools
uplevel #0 {
...
array set Curvature {
    types { NCurve }
    command { curvatNC -u 0.5; curvatNC -r -u 0.5 }
}
array set Torsion {
    types { NCurve }
    command { torsionNC -u 0.5; torsionNC -r -u 0.5 }
}
array set CurvatureNP {
    types { NPatch }
    command { curvatNP -u 0.5 -v 0.5; curvatNP -r -u 0.5 -v 0.5 }
}
array set IsPlanar {
    command { isPlanar }
}
...
array set GetPlaneNormal {
    command { getPlaneNormal ; getPlaneNormal -t }
}
...
foreach tool { IsPlanar IsCurve IsSurface IsDegen IsClosed IsParent HasChild
    IsTrimmed GetPlaneNormal } {
    set ${tool}(types) $::testDefaultCallbacksItems
    set ${tool}(precmd) $trimic
}
...
# test interrogating objects
puts $log "Testing interrogating objects ...\n"
foreach tool $aytesttools {
    puts $log "Testing $tool ...\n"
    puts -nonewline "${tool}, "
    update
...
proc aytest_var { type } {
  ...
  while {[info exists ::${type}_$i]} {
	  ...
	  if { [info exists ::${type}_${i}(freevars)] } {
		  ...
		  # compute total number of objects to be created
		  # for this variant
		  set countbody $body
		  set ::tot 0
		  append countbody "\{incr ::tot;\}"
		  catch {eval $countbody}
		  if { [llength $fixedvals] > 1 } {
		      set ::tot [expr $::tot * [llength $fixedvals]]
		  }		  
		  update
	      }
...
lappend items ApproxNPU ApproxNPV ApproxNPUV ApproxNPVU
...
set items {}
lappend items cbox tcircle tcone spiral helix extruden tsurf dualsweep hdisk
set testScriptObjectsItems $items

# set up script object types (0, 1, 2 - run, create, modify)
# corresponding to each test item
set types {}
lappend types 1 1 1 1 1 2 2 2 1
set testScriptObjectsTypes $types

# set up items to test in test #9
set items {}
lappend items Curvature Torsion CurvatureNP
lappend items IsCurve IsSurface IsPlanar IsDegen IsClosed IsTrimmed
lappend items HasChild IsParent GetPlaneNormal
set testInterrogatingObjectsItems $items
