...
proc Tree::_draw_tree { path } {
    ...
    set io [expr int([tk scaling]) * 8]
    set yl [_draw_subnodes $path [lrange $data(root) 1 end] $io \
	...
	set lw [expr int([tk scaling] * 1)]
	set id [$path:cmd create line $io $y0 $io $yl \
                    -fill    [Widget::getoption $path -linesfill]   \
                    -stipple [Widget::getoption $path -linestipple] \
		    -width $lw \
                    -tags    line]
...
bind $ay(tree) <Configure> "+\
	after idle \"Tree::_draw_tree $ay(tree);$ay(tree) selection set \[$ay(tree) selection get\]\""
