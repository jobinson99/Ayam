...
typedef struct ay_object_s {
  struct ay_object_s *next; /**< siblings of this object */
...
/** APatch object */
typedef struct ay_apatch_object_s {
  int mode; /**< approximation mode (U, V, UV, VU) */
  int width; /**< width of patch (U) */
  int height; /**< height of patch (V) */
  int awidth; /**< width of patch (U) */
  int aheight; /**< height of patch (V) */
  int close_u; /**< is patch closed in U? */
  int close_v; /**< is patch closed in V? */
  int order_u; /**< desired (approximation) order (U) */
  int order_v; /**< desired (approximation) order (V) */
...
typedef struct ay_gordon_object_s {
  ...
  int is_triangular; /**< triangular Gordon surface? */
  ...
  /** cached tesselated triangular Gordon surface */
  ay_object *pomesh;
...
typedef struct ay_view_object_s
{
  ...
  int full_notify; /**< controls scope of next notification */
} ay_view_object;
...
typedef struct ay_preferences_s
{
  ...
  /* modelling prefs */
  int scopemanagement; /**< explicit or implicit management? */
  ...
  int createat; /**< create objects at 0 - world origin, 1 - the mark, 2 - camera aim point */
  int createin; /**< create objects in world CS, 1 - view CS, 2 - camera CS */
  ...
  double guiscale; /**< GUI scaling factor (1.0 - 3.0) */

  /* drawing */
  int useguiscale; /**< adapt drawing to gui scale */
  ...
  double selbndfactor; /**< additional width of selected boundary curves */
  ...
  ay_view_object *createview; /**< view to create objects "in" (see createin) */
} ay_preferences;
...
/** Peek callback */
typedef int (ay_peekcb) (ay_object *o, unsigned int type,
			 ay_object **result, double *transform);
...
extern unsigned int ay_wrib_primlevel;
...
extern unsigned int ay_sbc_tagtype;
extern char *ay_sbc_tagname;
extern unsigned int ay_peek_tagtype;
extern char *ay_peek_tagname;
extern unsigned int ay_da_tagtype;
extern char *ay_da_tagname;
/*@}*/
...
#define AY_IDDSKIN         49
#define AY_IDAPATCH        50
...
/*@{*/
#define AY_VERSIONSTR "1.31"
#define AY_VERSIONSTRMI "0"

#define AY_VERSIONMA 1
#define AY_VERSION   31
