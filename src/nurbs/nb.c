...
ay_nb_SolveTridiagonal(int n, double *Q, double *U, double *P)
{
 ...
 int i, a;
  ...
  if((ay_status = ay_nb_BasisFuns(4, U[4], 3, U, abc)))
    goto cleanup;
  ...
  for(i = 3; i < n; i++)
    {
      ...
      if((ay_status = ay_nb_BasisFuns(i+2, U[i+2], 3, U, abc)))
	goto cleanup;
  ...
  if((ay_status = ay_nb_BasisFuns(n+2, U[n+2], 3, U, abc)))
    goto cleanup;
...
{
  return (12 +
	  ((p+1)+((p+1)+(p+1)+((p+1)*(p+1))+2*(p+1))) +
	  ((q+1)+((q+1)+(q+1)+((q+1)*(q+1))+2*(q+1))) +
	  4*(q+1) + 20);
} /* ay_nb_FirstDerSurf4DMSize */
 ...
 *
 * Memory management optimized variant, but C must be of size
 * (12 + ((p+1)+((p+1)+(p+1)+((p+1)*(p+1))+2*(p+1))) +
 * ((q+1)+((q+1)+(q+1)+((q+1)*(q+1))+2*(q+1))) + 4*(q+1) + 20)
 *
 * Furthermore:
		      ...
		      double *Pw, double u, double v, double *C)
{
 ...
 double w[3] = {0}, w2[3] = {0};
  ...
  Nu = &(C[12]);
  Nv = Nu + ((p+1)+((p+1)+(p+1)+((p+1)*(p+1))+2*(p+1)));
  temp = Nv + ((q+1)+((q+1)+(q+1)+((q+1)*(q+1))+2*(q+1)));
  Ct = temp + (q+1)*4;
  bin = Ct + 4*4;
 ...
 * Memory management optimized variant, but C must be of size
 * (12 + ((p+1)+((p+1)+(p+1)+((p+1)*(p+1))+2*(p+1))) +
 * ((q+1)+((q+1)+(q+1)+((q+1)*(q+1))+2*(q+1))) + 3*(q+1))
 *
 * Furthermore:
 * if C[0] is not 0, it already contains the u-span
 * if C[1] is not 0, it already contains the v-span
 ...
 * compute the second derivatives of rational surface
 * (n, m, p, q, U[], V[], Pw[]) at parametric values u,v in
 * C[24]: C[0] - point, C[3] - 1st der along u, C[6] - 2nd der along u,
 *        C[9] - 1st der along v, C[18] - 2nd der along v
 */
 ...
 * (w, h, q, V, Pw) if new surface does not deviate <tol> distance from old
