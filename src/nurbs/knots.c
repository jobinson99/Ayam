...
ay_knots_mergenc(ay_nurbcurve_object *curve, double *Ubar, int Ubarlen)
{
 ...
 int r, ia, ib, done;
  ...
  ay_nb_RefineKnotVectCurve(curve->is_rat, curve->length-1,
 ...
 int r, ia, ib, done;
  ...
  if(Ubar)
      ...
      else
	{
	  ...
	  ay_nb_RefineKnotVectSurfU(patch->is_rat,
  ...
  if(Vbar)
		       ...
		       sizeof(double))))
	{
      ...
      ay_nb_RefineKnotVectSurfV(patch->is_rat,
				patch->width-1, patch->height-1,
 ...
 *
 * \param[in] fpname  array of function and parameter name for error
 *                    reporting, may be NULL to inhibit reporting
 * \param[in] u  input parameter value
 * \param[in] relative  whether to interpret \a u in a relative way
 * \param[in] length  size of respective curve/surface dimension
 * \param[in] order  order of respective curve/surface
 * \param[in] knotv  knot vector
 * \param[in,out] v  where to store the resulting parameter value
 *
 * \returns AY_OK on success, error code otherwise
 */
int
ay_knots_evalparam(char **fpname, double u, int relative, int length, int order,
		   double *knotv, double *v)
{
 int ay_status = AY_OK;
 double ru;
  ...
  if(u != u)
    {
      if(fpname)
	ay_error_reportnan(fpname[0], fpname[1]);
      return AY_ERROR;
    }
  ...
  if(relative)
	...
	if(u == 1.0)
	  ...
	  ru = knotv[order-1] + (knotv[length] - knotv[order-1]) * u;
    }
  else
    {
      ru = u;
  ...
  if((ru < knotv[order-1]) ||
    ...
    {
      if(fpname)
	(void)ay_error_reportdrange(fpname[0], fpname[1],
				    knotv[order-1],
				    knotv[length]);
      return AY_ERROR;
    }
