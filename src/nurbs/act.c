 ...
 */
int
ay_act_leastSquares(double *Q, int m, int n, int p, int kt,
		    double **U, double **P)
{
 int ay_status = AY_OK;
 ...
 double *funs = NULL;
  ...
  if(kt)
    ay_status = ay_knots_centriparam(Q, m/*Qlen*/, istride, &ub);
  else
 ...
 */
int
ay_act_leastSquaresClosed(double *Q, int m, int n, int p, int kt,
			  double **U, double **P)
{
 int ay_status = AY_OK;
  ...
  while(i+1 < argc)
      ...
      if(argv[i] && argv[i][0] != '\0')
	{
	  switch(argv[i][1])
		...
		{
		  ay_error(AY_ERROR, argv[0], "Order must be > 1.");
		  return TCL_OK;
		}
	      break;
	    case 'p':
	    ...
	    case 't':
	      ...
	      if(tesselate < 0)
		{
		  ay_error(AY_ERROR, argv[0], "Tesselate must be >= 0.");
		  return TCL_OK;
		}
	      break;
	    default:
  ...
  while(sel)
      ...
      if(o->type == AY_IDNCURVE)
	{
	  ...
	  if(closed)
	      ...
	      if(length < (order*2))
		{
		  ay_error(AY_ERROR, argv[0],
			   "Length too small for closed curve.");
		  break;
		}
	    }
	  ...
	  if(tesselate)
	    ...
	    {
	      /* tesselate = 0 => do not tesselate at all */
	      if(!(tcv = malloc(curve->length*3*sizeof(double))))
		{
		  ay_error(AY_EOMEM, argv[0], NULL);
		  break;
		}
	      for(i = 0; i < curve->length; i++)
		memcpy(&(tcv[i*3]), &(curve->controlv[i*4]), 3*sizeof(double));
	      tlen = curve->length;
	    }
