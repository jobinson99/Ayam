 ...
 * Get memory address of a single IPatch control point from its indices
...
ay_ipt_interpolateu(ay_nurbpatch_object *np, int order, int ktype)
{
 ...
 double *U = NULL, *Q = NULL, total, d;
  ...
  if(!(U = calloc(K+order, sizeof(double))))
    {
...
ay_ipt_interpolatev(ay_nurbpatch_object *np, int order, int ktype)
{
 ...
 double *V = NULL, total, d;
  ...
  if(!(V = calloc(N+order, sizeof(double))))
    {
		     ...
		     double *sd, double *ed)
{
 ...
 double ds[3] = {0}, de[3] = {0};
  ...
  if(!(V = calloc(K+order, sizeof(double))))
    {
      ay_status = AY_EOMEM;
      goto cleanup;
    }
