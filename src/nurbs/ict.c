 ...
 *
 * \param[in,out] c  where to store the resulting NURBS curve
 *
 * \returns AY_OK on success, error code otherwise.
 */
int
 ...
 * Get memory address of a single ICurve control point from its index
 * (performing bounds checking).
 *
 * \param[in] curve  interpolating curve object to process
 * \param[in] index  index of desired control point
 * \param[in,out] p  address where to store the resulting memory address
 *
 * \returns AY_OK on success, error code otherwise.
 */
int
