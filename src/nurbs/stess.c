...
int ay_stess_AddBoundaryTrim(ay_object *o, int *added);
  ...
  *Clen = ((4 + n) * qf);
  ...
  if(is_rat)
      ...
      for(l = 0; l < (*Clen) + mc; l++)
	{
	  ...
	  have_zero_weight = AY_FALSE;
	  for(j = 0; j <= p; j++)
	    {
	      k = (span-p+j)*4;
	      if(fabs(Pw[k+3]) < AY_EPSILON)
		{
		  have_zero_weight = AY_TRUE;
		  break;
		}
	    }
...
ay_stess_SurfacePoints3D(int n, int m, int p, int q, double *U, double *V,
			 double *P, int qfu, int qfv,
			 int *Cn, int *Cm, double **C)
{
 int ay_status = AY_OK;
...
/* ay_stess_SurfacePoints3D2:
 *   calculate all points of an untrimmed NURBS surface
 */
int
ay_stess_SurfacePoints3D2(int n, int m, int p, int q, double *U, double *V,
			  double *P, int qfu, int qfv,
			  int *Cn, int *Cm, double **C)
{
 int ay_status = AY_OK;
 int spanu = 0, spanv = 0, i = 0, j = 0;
 int a, b, aa, bb;
 int nu, nv;
 double u, v, lastu, lastv, ud, vd;
 double temp[3] = {0}, *Ct = NULL, *fder = NULL, *fd1, *fd2;
 double *uknots, *vknots, *uspans = NULL, *vspans = NULL;
 double *knotsnspans = NULL;


  if(!(knotsnspans = malloc(((n + p + 3)*2 + (m + q + 3)*2) * sizeof(double))))
    { ay_status = AY_EOMEM; goto cleanup; }

  uknots = knotsnspans;
  uspans = &(knotsnspans[n+p+3]);
  vknots = &(knotsnspans[(n+p+3)*2]);
  vspans = &(knotsnspans[(n+p+3)*2+m+q+3]);
  ...
  *Cm = j+(j-1)*qfv;
  ...
  aa = 1;
  ud = (uknots[1]-uknots[0])/(qfu+1);
  u = uknots[0];
  j = 0;
  for(a = 0; a < (*Cn); a++)
    {
      bb = 1;
      vd = (vknots[1]-vknots[0])/(qfv+1);
      v = vknots[0];
      ...
      for(b = 0; b < (*Cm); b++)
	{
	  ...
	  if(v > (vknots[bb]-AY_EPSILON))
	    {
	      v = vknots[bb];
	      bb++;
	      vd = (vknots[bb]-vknots[bb-1])/(qfv+1);
	    }
	} /* for */

      u += ud;

      if(u > (uknots[aa]-AY_EPSILON))
	{
	  u = uknots[aa];
	  aa++;
	  ud = (uknots[aa]-uknots[aa-1])/(qfu+1);
	}
 ...
 * \param[in,out] added  will be set to AY_TRUE if the boundary trim was added
 *
 * \returns AY_OK on success, error code otherwise.
 */
int
ay_stess_AddBoundaryTrim(ay_object *o, int *added)
{
 int ay_status = AY_OK;
 ay_object *c = NULL;
 ay_nurbpatch_object *np = NULL;
 ...
 int is_bound;
  ...
  ay_status = ay_npt_isboundcurve(o->down, umin, umax, vmin, vmax, &is_bound);
  ...
  if(!is_bound)
	...
	{
	  c = ay_peek_singleobject(o->down, AY_IDNCURVE);
	  if(c)
	    {
...
ay_stess_TessTrimmedNP(ay_object *o, int qf, ay_stess_patch *stess)
{
 ...
 int boundadded = AY_FALSE;

  np = (ay_nurbpatch_object *)o->refine;

  ay_stess_destroy(stess);

  ay_status = ay_stess_AddBoundaryTrim(o, &boundadded);
 ...
 *
 * \param[in] stess  STESS tesselation
 * \param[in,out] result  where to store the new polymesh object
 *
 * \returns AY_OK on success, error code otherwise.
 */
int
ay_stess_topomesh(ay_stess_patch *stess, ay_object **result)
{
 int ay_status = AY_OK;
 ay_copycb *cb;
 ay_stess_uvp *u1, *u2;
 double *cv = NULL;
 int ii, jj;
 unsigned int i, a, b, n, m, npolys, v;
 ...
 ay_object *new;
  ...
  if(stess->pomesh)
    {
      cb = (ay_copycb*)ay_copycbt.arr[AY_IDPOMESH];
      ay_status = cb(stess->pomesh, &(new->refine));
    }
  else
    {
      if(!(po = calloc(1, sizeof(ay_pomesh_object))))
	{
      ...
      if(stess->tessv)
	{
	  if(!(cv = malloc(6*stess->tessw*stess->tessh*sizeof(double))))
	    {
	      ay_status = AY_EOMEM;
	      goto cleanup;
	    }
	  memcpy(cv, stess->tessv, 6*stess->tessw*stess->tessh*sizeof(double));
	  npolys = (stess->tessw-1) * stess->tessh * 2;

	  if(!(nloops = malloc(npolys * sizeof(unsigned int))))
	    {
	      ay_status = AY_EOMEM;
	      goto cleanup;
	    }
	  for(i = 0; i < npolys; i++)
	    nloops[i] = 1;
	  if(!(nverts = malloc(npolys * sizeof(unsigned int))))
	    {
	      ay_status = AY_EOMEM;
	      goto cleanup;
	    }
	  for(i = 0; i < npolys; i++)
	    nverts[i] = 3;
	  if(!(verts = malloc(npolys * 3 * sizeof(unsigned int))))
	    {
	      ay_status = AY_EOMEM;
	      goto cleanup;
	    }
	  ...
	  b = stess->tessh;
	  n = 0;
	  for(ii = 0; ii < stess->tessw-1; ii++)
	    {
	      for(jj = 0; jj < stess->tessh-1; jj++)
		{
		  verts[n] = a;
		  verts[n+1] = a+1;
		  verts[n+2] = b;
	  ...
	  n = stess->tessw*stess->tessh;
	}
      else
	{
	  n = (stess->upslen-1) * (stess->vpslen-1) * 2;
	  /* 3 * 2 * 3 */
	  /* 3 (points per tri) * 2 (coord + normal) * 3 (components) */
	  if(!(cv = malloc(n*18*sizeof(double))))
	    {
	      ay_status = AY_EOMEM;
	      goto cleanup;
	    }
	  if(!(nloops = malloc(n * sizeof(unsigned int))))
	    {
	      ay_status = AY_EOMEM;
	      goto cleanup;
	    }
	  for(i = 0; i < n; i++)
	    nloops[i] = 1;
	  if(!(nverts = malloc(n * sizeof(unsigned int))))
	    {
	      ay_status = AY_EOMEM;
	      goto cleanup;
	    }
	  for(i = 0; i < n; i++)
	    nverts[i] = 3;
	  if(!(verts = malloc(n * 3 * sizeof(unsigned int))))
	    {
	      ay_status = AY_EOMEM;
	      goto cleanup;
	    }
	  ...
	  for(ii = 0; ii < (stess->upslen-1); ii++)
	      ...
	      while(u1 && u1->next && u2 && u2->next)
		{
		  if(u1->type == 0 && u1->next->type == 0 &&
		     ...
		     u1->v == u2->v)
		    {
		      memcpy(&(cv[m]), u1->C, 6*sizeof(double));
		      m += 6;
		      memcpy(&(cv[m]), u1->next->C, 6*sizeof(double));
		      m += 6;
		      memcpy(&(cv[m]), u2->C, 6*sizeof(double));
		      m += 6;
		      memcpy(&(cv[m]), u2->next->C, 6*sizeof(double));
		      m += 6;
		      u1 = u1->next;
		      u2 = u2->next;
