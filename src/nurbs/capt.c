 ...
 *                     at the corresponding boundary, we do not create the
 *                     cap here but later when creating the bevel
 ...
 *                     (may stay empty if all caps are integrated into
 *                     the surface)
 *
 ...
 * \param[in,out] cap  where to store the new NURBS patch object
 *
 * \returns AY_OK on success, error code otherwise.
 */
int
 ...
 *
 * \param[in,out] c  NURBS curve object(s); multiple objects may be
 *                   provided as list of connected objects; the first curve
 *                   must be the outmost curve, all following curves define
 *                   holes; the curve orientations will be detected and
 *                   adjusted properly
 * \param[in,out] cap  where to store the new NURBS patch object
 *
 * \returns AY_OK on success, error code otherwise.
 ...
 * the supplied curve object will be modified (split)!
 *
