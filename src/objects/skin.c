...
ay_skin_drawhcb(struct Togl *togl, ay_object *o)
{
 ay_skin_object *skin;
  ...
  ay_npt_drawhandles(togl, (ay_nurbpatch_object *)skin->npatch->refine);
...
ay_skin_setpropcb(Tcl_Interp *interp, int argc, char *argv[], ay_object *o)
{
 ...
 ay_skin_object *skin = NULL;
  ...
  if((to = Tcl_GetVar2Ex(interp, arr, "Interpolate",
...
ay_skin_getpropcb(Tcl_Interp *interp, int argc, char *argv[], ay_object *o)
{
 ...
 ay_skin_object *skin = NULL;
  ...
  Tcl_SetVar2Ex(interp, arr, "Interpolate",
		Tcl_NewIntObj(skin->interpolate),
		TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY);

  Tcl_SetVar2Ex(interp, arr, "Order_U",
		Tcl_NewIntObj(skin->uorder),
		TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY);

  Tcl_SetVar2Ex(interp, arr, "Knot-Type_U",
		Tcl_NewIntObj(skin->uknot_type),
		TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY);

  Tcl_SetVar2Ex(interp, arr, "Tolerance",
		Tcl_NewDoubleObj(skin->glu_sampling_tolerance),
		TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY);

  Tcl_SetVar2Ex(interp, arr, "DisplayMode",
		Tcl_NewIntObj(skin->display_mode),
		TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY);

  ay_prop_getnpinfo(interp, arr, skin->npatch);
...
ay_skin_notifycb(ay_object *o)
{
 ...
 int mode = 0, count = 0, phase = -1, i, a, ktype;
  ...
  if(skin->interpolate)
    {
      patch = (ay_nurbpatch_object *)newo->refine;
      if(patch->width > 2 && patch->uorder > 2)
	{
	  ktype = AY_KTCHORDAL;
	  if(skin->uknot_type == AY_KTCENTRI)
	    ktype = AY_KTCENTRI;
	  else
	    if(skin->uknot_type == AY_KTUNIFORM)
	      ktype = AY_KTUNIFORM;
	  ay_status = ay_ipt_interpolateu(patch, patch->uorder, ktype);
	  if(ay_status)
	    {
...
cleanup:
  ...
  if(o->selp)
    {
      if(skin->npatch)
	(void)ay_skin_getpntcb(3, o, NULL, NULL);
      else
	ay_selp_clear(o);
    }
  ...
  if(ay_status)
	...
	{
	case 0:
	  ay_error(AY_ERROR, fname, "Getting parameter curves failed.");
	  break;
	case 1:
	  ay_error(AY_ERROR, fname, "Skin failed.");
	  break;
	case 2:
	...
	case 3:
	  ...
	  break;
	case 4:
	  ay_error(AY_EWARN, fname, "Bevel creation failed.");
	  ay_status = AY_OK;
	  break;
	default:
	  break;
