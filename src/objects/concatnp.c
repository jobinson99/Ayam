...
ay_concatnp_drawhcb(struct Togl *togl, ay_object *o)
{
 ay_concatnp_object *concatnp = NULL;
  ...
  ay_npt_drawhandles(togl, (ay_nurbpatch_object *)concatnp->npatch->refine);
...
ay_concatnp_setpropcb(Tcl_Interp *interp, int argc, char *argv[], ay_object *o)
{
 ...
 int stringlen, newknottype = 0;
  ...
  if((to = Tcl_GetVar2Ex(interp, arr, "Compatible",
			 TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    Tcl_GetIntFromObj(interp, to, &(concatnp->compat));

  if((to = Tcl_GetVar2Ex(interp, arr, "UVSelect",
			 TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    {
  ...
  if((to = Tcl_GetVar2Ex(interp, arr, "Tolerance",
			 TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    Tcl_GetDoubleFromObj(interp, to, &(concatnp->glu_sampling_tolerance));

  if((to = Tcl_GetVar2Ex(interp, arr, "DisplayMode",
			 TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    Tcl_GetIntFromObj(interp, to, &(concatnp->display_mode));
...
ay_concatnp_getpropcb(Tcl_Interp *interp, int argc, char *argv[], ay_object *o)
{
 ...
 Tcl_Obj *to = NULL;
  ...
  Tcl_SetVar2Ex(interp, arr, "Tolerance",
		Tcl_NewDoubleObj(concatnp->glu_sampling_tolerance),
		TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY);

  Tcl_SetVar2Ex(interp, arr, "DisplayMode",
		Tcl_NewIntObj(concatnp->display_mode),
		TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY);
...
ay_concatnp_readcb(FILE *fileptr, ay_object *o)
{
 ...
 ay_concatnp_object *concatnp = NULL;
  ...
  if(ay_read_version >= 17)
    {
      /* Since Ayam 1.30: */
      fscanf(fileptr, "%d\n", &concatnp->display_mode);
      fscanf(fileptr, "%lg\n", &concatnp->glu_sampling_tolerance);
    }
