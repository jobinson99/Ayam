...
ay_npatch_drawtrimglu(GLUnurbsObj *no, ay_object *o, ay_object *t)
{
 ...
 double tm[16];
  ...
  if(AY_ISTRAFO(t))
    {
      ay_trafo_creatematrix(t, tm);
      c = &(ncurve->fltcv[knot_count]);
      if(ncurve->is_rat)
	stride = 4;
      for(i = 0; i < ncurve->length; i++)
	{
	  d[0] = tm[0]*c[0] + tm[4]*c[1] + tm[8] *c[2] + tm[12]*1.0;
	  d[1] = tm[1]*c[0] + tm[5]*c[1] + tm[9] *c[2] + tm[13]*1.0;
	  d[2] = tm[2]*c[0] + tm[6]*c[1] + tm[10]*c[2] + tm[14]*1.0;
	  memcpy(c, d, 3*sizeof(float));
	  c += stride;
	} /* for */
    } /* if */
  ...
  gluNurbsProperty(no, GLU_SAMPLING_METHOD, GLU_DOMAIN_DISTANCE);
  gluNurbsProperty(no, GLU_U_STEP, (GLfloat)200);
...
ay_npatch_setpropcb(Tcl_Interp *interp, int argc, char *argv[], ay_object *o)
{
 ...
 ay_object *b;
 ay_nurbpatch_object *npatch = NULL;
 int new_uorder = -1, new_width = -1, new_uknot_type = -1, uknots_modified = 0;
 int new_vorder = -1, new_height = -1, new_vknot_type = -1, vknots_modified = 0;
 ...
 int knotc, i;
  ...
  if((to = Tcl_GetVar2Ex(interp, arr, "Knots_U-Modified",
			 TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    Tcl_GetIntFromObj(interp, to, &uknots_modified);

  if((to = Tcl_GetVar2Ex(interp, arr, "Knots_V-Modified",
			 TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
		  ...
		  if(tcl_status == TCL_OK)
		  ...
		  else
		    {
		      ay_error(AY_ERROR, fname,
			       "Could not decode new knot list:");
		      ay_error(AY_ERROR, fname,
			       Tcl_GetStringFromObj(to, NULL));
		    }
