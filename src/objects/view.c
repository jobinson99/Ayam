...
ay_view_setpropcb(Tcl_Interp *interp, int argc, char *argv[], ay_object *o)
{
 ...
 char *ma[3] = {"Mark_X","Mark_Y","Mark_Z"};
  ...
  arr = arv;
  if((tco = Tcl_GetVar2Ex(interp, arr, "Type",
			  TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    {
  ...
  if((tco = Tcl_GetVar2Ex(interp, arr, "Redraw",
			  TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    Tcl_GetIntFromObj(interp, tco, &view->redraw);

  if((tco = Tcl_GetVar2Ex(interp, arr, "DrawingMode",
			  TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    Tcl_GetIntFromObj(interp, tco, &view->drawmode);

  if((tco = Tcl_GetVar2Ex(interp, arr, "DrawSel",
			  TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    Tcl_GetIntFromObj(interp, tco, &view->drawsel);

  if((tco = Tcl_GetVar2Ex(interp, arr, "DrawLevel",
			  TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    Tcl_GetIntFromObj(interp, tco, &view->drawlevel);

  if((tco = Tcl_GetVar2Ex(interp, arr, "DrawObjectCS",
			  TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    Tcl_GetIntFromObj(interp, tco, &view->drawobjectcs);

  if((tco = Tcl_GetVar2Ex(interp, arr, "AALines",
			  TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    Tcl_GetIntFromObj(interp, tco, &view->antialiaslines);

  if((tco = Tcl_GetVar2Ex(interp, arr, "GridSize",
			  TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    Tcl_GetDoubleFromObj(interp, tco, &view->grid);

  if((tco = Tcl_GetVar2Ex(interp, arr, "DrawGrid",
			  TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
  ...
  if((tco = Tcl_GetVar2Ex(interp, arr, "ModellingMode",
			  TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    Tcl_GetIntFromObj(interp, tco, &view->local);

  if((tco = Tcl_GetVar2Ex(interp, arr, "TransformPoints",
			  TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    Tcl_GetIntFromObj(interp, tco, &view->transform_points);

  if((tco = Tcl_GetVar2Ex(interp, arr, "EnableUndo",
			  TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    Tcl_GetIntFromObj(interp, tco, &view->enable_undo);

  if((tco = Tcl_GetVar2Ex(interp, arr, "SetMark",
			  TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
  ...
  if(setmark)
    {
      for(i = 0; i < 3; i++)
	{
	  if((tco = Tcl_GetVar2Ex(interp, arr, ma[i],
				  TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
  ...
  if((tco = Tcl_GetVar2Ex(interp, arr, "DrawBG",
			  TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
