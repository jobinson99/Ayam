...
ay_pamesh_drawhcb(struct Togl *togl, ay_object *o)
{
 ...
 int width, height, i;
  ...
  if(view->drawhandles == 2)
    {
      if(pm->npatch)
	ay_npt_drawbreakpoints((ay_nurbpatch_object *)pm->npatch->refine);
      return AY_OK;
    }
...
ay_pamesh_setpropcb(Tcl_Interp *interp, int argc, char *argv[], ay_object *o)
{
 ...
 Tcl_Obj *to = NULL;
 ay_pamesh_object *pamesh = NULL;
 int new_close_u = -1, new_width = -1, new_btype_u = -1, new_step_u = -1;
 int new_close_v = -1, new_height = -1, new_btype_v = -1, new_step_v = -1;
 int new_type = -1, i, j, update = AY_FALSE;
 double dtemp, *basis;
  ...
  if((to = Tcl_GetVar2Ex(interp, arr, "BType_U",
			 TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    Tcl_GetIntFromObj(interp, to, &(new_btype_u));

  if((to = Tcl_GetVar2Ex(interp, arr, "BType_V",
			 TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
  ...
  if(new_btype_u == AY_BTCUSTOM)
      ...
      if(pamesh->btype_u == AY_BTCUSTOM)
	{
	  for(j = 0; j < 16; j++)
	    {
	      if((to = Tcl_ObjGetVar2(interp, arrobj, bmu[j],
				      TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
		{
		...
		{
		  ay_error(AY_ERROR, fname, "Could not decode basis matrix u.");
		  goto cleanup;
		}
	    } /* for */
	}
      else
	{
      ...
      if((to = Tcl_GetVar2Ex(interp, arr, "Step_U", TCL_LEAVE_ERR_MSG |
			     TCL_GLOBAL_ONLY)))
	{
  ...
  if(new_btype_v == AY_BTCUSTOM)
      ...
      if(pamesh->btype_v == AY_BTCUSTOM)
	{
	  for(j = 0; j < 16; j++)
	    {
	      if((to = Tcl_ObjGetVar2(interp, arrobj, bmv[j],
				      TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
		{
		  Tcl_GetDoubleFromObj(interp, to, &dtemp);
		...
		{
		  ay_error(AY_ERROR, fname, "Could not decode basis matrix v.");
		  goto cleanup;
		}
	    } /* for */
	}
      else
	{
      ...
      if((to = Tcl_GetVar2Ex(interp, arr, "Step_V",
			     TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
	{
...
ay_pamesh_wribcb(char *file, ay_object *o)
{
 ...
 unsigned int ci = 1;
  ...
  if(!(pvc = ay_pv_count(o)))
    {
      /* No */
      RiPatchMesh(type, nu, uwrap, nv, vwrap, "Pw", controls, RI_NULL);
    }
  else
    {
