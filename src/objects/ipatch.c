...
ay_ipatch_setpropcb(Tcl_Interp *interp, int argc, char *argv[], ay_object *o)
{
 ...
 Tcl_Obj *to = NULL;
 ay_ipatch_object *ipatch = NULL;
 int new_ktype_u = -1, new_close_u = -1, new_order_u = -1, new_width = -1;
 int new_ktype_v = -1, new_close_v = -1, new_order_v = -1, new_height = -1;
 int i, update = AY_FALSE;
  ...
  if((to = Tcl_GetVar2Ex(interp, arr, "Derivatives_U",
			 TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    Tcl_GetIntFromObj(interp, to, &(ipatch->derivs_u));

  if((to = Tcl_GetVar2Ex(interp, arr, "SDLen_U",
			 TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    Tcl_GetDoubleFromObj(interp, to, &(ipatch->sdlen_u));

  if((to = Tcl_GetVar2Ex(interp, arr, "EDLen_U",
			 TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    Tcl_GetDoubleFromObj(interp, to, &(ipatch->edlen_u));

  if((to = Tcl_GetVar2Ex(interp, arr, "Derivatives_V",
			 TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    Tcl_GetIntFromObj(interp, to, &(ipatch->derivs_v));

  if((to = Tcl_GetVar2Ex(interp, arr, "SDLen_V",
			 TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
    Tcl_GetDoubleFromObj(interp, to, &(ipatch->sdlen_v));

  if((to = Tcl_GetVar2Ex(interp, arr, "EDLen_V",
			 TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY)))
